...
describe('AlarmRow component', () => {
    test('renders alarm data correctly', () => {
    ...
    render(<AlarmRow alarm={alarm} isSelected={false} handleSelectAlarm={() => {}} />);     // renderiza la fila de la alarma
...
// Aplica correctamente la clase CSS cuando isSelected es truec
test('applies selected class when isSelected is true', () => {
    const alarm = { id: '123' };    
    
    render(<AlarmRow alarm={alarm} isSelected={true} handleSelectAlarm={() => {}} />);
...
test('applies unselected class when isSelected is false', () => {
    ...
    render(<AlarmRow alarm={alarm} isSelected={true} handleSelectAlarm={() => {}} />);
    const row = screen.getByRole('row'); 
    expect(row).toHaveClass('bg-african_violet-400');    
});
...
test('calls handleSelectAlarm with the correct alarm when clicked', () => {
    ...
    render(<AlarmRow alarm={alarm} isSelected={false} handleSelectAlarm={handleSelectAlarm} />);
...
test('row has cursor-pointer class', () => {
    ...
    render(<AlarmRow alarm={alarm} isSelected={false} handleSelectAlarm={() => {}} />);
...
test('renders correctly with incomplete alarm data', () => {
    ...
    expect(screen.getByText('123')).toBeInTheDocument();
    expect(screen.getByText('AAPL')).toBeInTheDocument();
    // Verifica que no arroja errores o muestra texto placeholder
});  
...
test('selects different alarms on click', () => {
  ...
  const { rerender } = render(<AlarmRow alarm={alarm1} isSelected={false} handleSelectAlarm={handleSelectAlarm} />);
  
  fireEvent.click(screen.getByText('123'));
  expect(handleSelectAlarm).toHaveBeenCalledWith(alarm1);
  
  rerender(<AlarmRow alarm={alarm2} isSelected={false} handleSelectAlarm={handleSelectAlarm} />);
